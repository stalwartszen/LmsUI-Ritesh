  ...
  return (
    <div>
      <div className="p-5 border-slate-700 flex flex-col gap-3 rounded-xl border bg-slate-900 ">
        <h1 className="text-lg my-3">Semester I</h1>
        {Data.map((item) => {
            return (
                <>
              <div>
                <ModuleComp key={item.id} props={item} />
              </div>
            </>
          );
        })}
  ...
  return (
    <>
      <div
        ...
        className="bg-slate-800 flex justify-between p-5 rounded-lg"
      >
        ...
        <button
          onClick={() =>
            navigate(`/admin/classroom/${SubjectId.id}/modules/${id}/test`)
          }
          className="btn-sm bg-blue-800 rounded-md"
        >
          take a test
        </button>
      </div>
